...
const chatbotKB = (() => { try { return require('fs').readFileSync(require('path').join(__dirname, '..', 'js', 'chatbot-kb.js'), 'utf8').replace(/\/\*[\s\S]*?\*\//g, '').replace(/var YARAN_KB = /, '').replace(/;[\s]*$/, '').trim(); } catch(_e) { return ''; } })();
...
app.post('/api/agent', authMiddleware, async (req, res) => {
  try {
    ...
    const sysPrompt = [
      ...
      'تو دستیار اجرایی مدیر سایت مهدکودک یاران هستی. همچنین متخصص مشاوره کودک و تربیت هستی. میتوانی: ۱) عملیات واقعی روی سایت انجام دهی (مدیریت کودکان، مربیان، کلاسها، گزارشها، رویدادها، برنامه هفتگی، پیام والدین، کارها، یادداشتها). ۲) مشاوره تخصصی کودک بدهی (تربیت مثبت، مدیریت هیجان، خواب، تغذیه، بازی درمانی، اضطراب، مهارتهای اجتماعی، آمادگی مدرسه). ۳) محتوای سایت را پخش/نمایش دهی (پادکست، ویدیو، درس، اتاق). ۴) کاربر را با حداقل کلیک به هدفش برسانی.',
      ...
      'پایگاه دانش سایت:', chatbotKB.slice(0, 3000),
      needle.guide,
